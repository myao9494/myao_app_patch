...
```
file_manager/
├── backend/           # FastAPI バックエンド (Port 8001)
│   ├── app/
│   │   ├── main.py    # エントリーポイント (/quick 静的配信含む)
│   │   ├── config.py  # 設定
│   │   └── routers/   # APIルーター
│   └── tests/         # pytest テスト
├── frontend/          # React フロントエンド (PC向けファイルマネージャー本体)
├── frontend-quick/    # React フロントエンド (スマホ特化クイック検索・編集アプリ)
│   └── src/
│       ├── api/       # APIクライアント
│       ├── components/# UIコンポーネント (検索、閲覧、編集、設定)
│       └── utils/     # 設定管理、電話番号抽出
├── docs/              # ドキュメント
└── CLAUDE.md          # このファイル
```

## Quick アプリ（スマホ特化クイック検索・編集）

スマートフォンからの端末内データアクセス・検索・連絡先取得・簡易編集に特化した独立フロントエンド。
本体に影響を与えない疎結合設計で、バックエンドの `/quick/` パスから配信されます。

- **URL**: `http://<ホスト>:8001/quick/` （Tailscale: `http://mineomacbook-air.taild3cb7c.ts.net:8001/quick/`）
- **検索**: `/Users/mine/000_work` を対象に、全文検索（Local-fulltext-search: 8079番、深さ20階層再帰）とファイル名検索をハイブリッド実行。キーワード一致箇所（`<mark>`）をカード内にハイライト表示
- **連絡先クイックアクション**: 本文内の電話番号を自動抽出し、ワンタップで電話発信（`tel:`）やコピーが可能
- **対応形式**: Markdown (`.md`), テキスト (`.txt`), Excalidraw (`.excalidraw`, `.excalidraw.md`), DrawIO/SVG (`.drawio.svg`, `.svg`)
- **レンダリング表示**: 見出し、チェックボックス、コードブロック、コールアウト、GFMテーブル（アライメント、インライン装飾、スマホ横スクロール対応）、URL自動リンク化（同一タブ遷移でブラウザの戻るボタンが利用可能）、Mermaidダイアグラム（```mermaid をSVGで動的レンダリング）、Obsidian画像構文（`![[image.png]]`、Vault内自動探索、相対パス）やExcalidraw図面埋め込み（`![[図.excalidraw|1475]]`）を含むフルレンダリング表示
- **Obsidian・Excalidrawスマート図面描画**:
  - Obsidian Vault ルート（`.obsidian`）の動的探索と Vault 相対パス（`01_data/...`）の自動解決
  - `![[図.excalidraw]]` 埋め込み時、Obsidian Excalidraw Cards と互換性のある FNV-1a ハッシュにより `.excalidraw-cache` のプレビューPNG/SVGを自動検出してインライン描画
  - `.excalidraw.md` ファイル直接オープン時、図面プレビュー画像表示 ＋ Excalidraw起動ボタン（Port 3001）を提供
- **履歴・ルーティング同期**: ファイル閲覧時にURLクエリパラメータ（`?file=...`）とHistory APIを同期。外部リンクからブラウザの「戻る」ボタンで戻った際、トップ画面に戻らず開いていたファイル画面が自動復元される
- **編集機能**: 閲覧画面からワンタップで編集モードへ切り替え、端末内ファイルを即時更新
- **ホスト設定**: UIの設定モーダル（⚙️）からTailscaleドメインやホスト名、ポートを柔軟に変更・保存可能
...
- 右クリックメニュー
  - HTMLファイル（.html, .htm）: **HTMLプレビューを開く**（ブラウザの新規タブでレンダリング表示）
  - プログラムコードファイル: コードで開く（VS Code）、エディターで開く（内蔵エディタ）、実行
  - Zipファイル: Zip解凍
  - 共通: 名前を変更、削除、コピー、切り取り、貼り付け
- **ペイン背景・テーブルヘッダー右クリックメニュー（PaneContextMenu）**
  - 余白部分やテーブルヘッダー（Name, Size, Date, Git等）の右クリックで表示
  - **リスト取得(check_box)**: 一覧をMarkdownチェックボックス形式（`- [ ] {name}`、フォルダも末尾スラッシュなし）でクリップボードにコピー
  - **リスト取得(箇条書き)**: 一覧をMarkdown箇条書き形式（`- {name}`、フォルダも末尾スラッシュなし）でクリップボードにコピー
  - **隣のペインで開く**: 現在のフォルダを隣のペイン（左ペインなら中央ペイン、中央ペインなら左ペイン）で開く（操作ペインのフォーカスは維持）
- Markdownエディタモーダル（新規作成・編集）
- **戻る/進むボタン**: カーソル位置と選択状態も履歴と一緒に保存・復元
- **キーボードショートカット**: Ctrl+Left(戻る), Ctrl+Right(進む), Ctrl+Up(上の階層へ), a(フォルダ追加、左/中ペインのみ), r(再読み込み/更新、左/中ペインのみ), n(リネーム、左/中ペインのみ)
...
| GET | /api/view-pdf | PDFインライン表示 | 済 |
| GET | /api/view-html | HTMLインラインプレビュー表示 | 済 |
...
## 実装済み機能（更新）
 ...
 4. **ファイルアップロード/ダウンロード**:
    ...
    - アップロード: **Explorer / Finderからのファイル・フォルダドラッグ＆ドロップ対応** (OSからのファイルおよびフォルダドロップに対応。`webkitGetAsEntry` APIによる再帰スキャンでサブフォルダ・ファイル・空フォルダの階層構造を保持してアップロード、バックエンド側での親階層自動作成・パストラバーサル防止を完備)
 ...
 10. **画像インライン表示と画像ビューワー (Image Viewer)**:
    - バックエンド API `GET /api/view-image?path=...` を実装。PNG, JPG, GIF, SVG, WEBP, BMP, ICO, AVIF を適切な MIME タイプとインラインヘッダーで配信。
    - フロントエンドに大画面画像プレビューモーダル (`ImagePreviewModal`) を実装。画像ダブルクリック・Enter・右クリック「画像を表示」で起動。
    - ズーム（拡大・縮小・実寸）、回転、同一フォルダ内の前後画像移動（`←` / `→`）、解像度・ファイルサイズ表示、ダウンロード、別タブ表示を完備。
 11. **左右ペイン同期移動 (Synchronized Browsing)**:
    - ヘッダー内の「同期移動」トグルボタンおよびショートカットキー `Alt + S` で有効化/解除可能（設定は localStorage に保存）。
    - 片方のペインでフォルダ移動（子フォルダへ進む、親フォルダへ戻る等）した際、もう一方のペインも連動して対応するフォルダへ移動。
    - 同一パス同期および相対サブフォルダ・親フォルダ同期をサポートし、無限ループ防止ガードを搭載。
    - 右ペインを左ペインのパスに瞬時に一致させる「左右パス同期」ボタンも提供。
 12. **検索ボックスの強化**:
    - `Ctrl/Cmd + F` または `/` キーでペイン内のファイル名検索ボックスに即座にフォーカス。
    - `Enter` で次のヒットへ移動、`Shift + Enter` で前のヒットへ戻る（循環ラップアラウンド、フォーカス維持で連続移動可能）。
    - リアルタイム件数・位置表示バッジ（検索中は `現在位置 / ヒット数`、未検索時は `全件 / 全件`）。
    - 入力中のクリアボタン（`×`）、正規表現トグル（`.*`）を実装。
    - `Escape` キーで検索クエリを全消去し、ファイル一覧テーブルへフォーカスを復帰。
 13. **Markdownビューワ／エディタの検索ボックス (Markdown Search)**:
    - `MarkdownEditorModal` に専用の検索バーを新設。
    - `Cmd/Ctrl + F` またはヘッダーの検索入力欄で即時フォーカス＆テキスト全選択。
    - `Enter` で次のマッチへ、`Shift + Enter` で前のマッチへ循環移動（`▲` / `▼` ボタンも完備）。
    - リアルタイム位置・件数表示バッジ（`現在位置 / 総件数`）、正規表現トグル（`.*`）、ワンクリッククリア（`×`）。
    - エディタペイン（Textarea）の選択範囲・スクロール追従と、プレビューペイン（HTML）の安全な `<mark>` ハイライト＆自動スクロールが完全連動。
    - `Escape` キーでクエリをクリアしエディタに復帰（モーダルは閉じない）。
 14. **Markdownビューワ／エディタの1行集約ヘッダー＆作業領域最大化**:
    - モーダル上部に散在していた複数段（タイトル、装飾ツールバー、検索バー、ショートカット説明）および下部フッター・ステータスバーを完全撤廃。
    - 最上部のタイトル行（横1行）に「タイトル」「スリム検索バー」「表示モード切替（Edit/Split/Preview）」「ステータス（行数・未保存/保存済）」「キャンセル・保存ボタン」をコンパクトに集約。
    - 不要な装飾アイコンや説明文を排除し、モーダル内余白を最小化（8px〜12px）することで、テキスト編集領域およびMarkdownプレビュー領域を上下左右いっぱいに最大化。
    - `Cmd/Ctrl + S`（保存）、`Cmd/Ctrl + B`（太字）、`Cmd/Ctrl + F`（検索）などのキーボードショートカットはすべてそのまま動作。
 15. **Markdownプレビュー内の画像表示（相対パス・絶対パス・Obsidian構文対応）**:
    - `![alt](image.png)` などのローカル相対パスをMarkdownファイルのディレクトリ基準で自動解決。
    - バックエンド `GET /api/view-image?path=...` を経由してプレビュー内にインライン画像を安全に描画。
    - Obsidian画像埋め込み構文（`![[image.png]]`、幅指定 `![[image.png|300]]`、幅・高さ指定 `![[image.png|300x200]]`）に完全対応。
    - 外部Web画像（HTTP/HTTPS）や Data URI 画像も表示。画像の最大幅100%制限によるレスポンシブ表示。
 16. **メールファイルアイコン (.msg, .eml) のCatppuccinメールアイコン対応**:
    - Outlookメッセージファイル（`.msg`）および電子メールメッセージファイル（`.eml`）に対して、封筒デザインのメールアイコン（`/icons/catppuccin/mail.svg`）を割り当て。
    - ファイル一覧（`FileList`）およびファイル検索（`FileSearch`）で直感的なメールアイコンが表示される。
 17. **ファイル一覧テーブルのセル垂直揃え・下線浮き上がり防止 (Table Alignment Fix)**:
    - `FileSearch.css` の `.name-cell` セレクタを `.results-table .name-cell` にスコープ化し、`FileList` へのスタイル汚染を防止。
    - `FileList.css` に `.file-table td.name-cell` の `display: table-cell; vertical-align: middle;` を明示保護。
    - 日付折り返しなどで行の高さが拡張された場合でも、チェックボックス、Name、Size、Dateの全列で垂直中央揃えおよび下線（`border-bottom`）が完全に行の最下端に美しく揃うよう改善。
 18. **Obsidian Vault図面描画・Excalidrawプレビュー・Mermaid描画（Quickアプリ）**:
    - **Obsidian Vault 内図面・画像解決**: Vault ルート（`.obsidian`）の動的探索と、Vault相対パス（`01_data/...`）の自動解決をバックエンド `/api/view-image` に実装。ファイル名のみ（`Pasted image ...png`）の指定でもVault内を再帰スキャンして即時配信。
    - **Excalidraw キャッシュ連携**: Obsidian Excalidraw Cards プラグイン互換の FNV-1a ハッシュアルゴリズムにより、`.excalidraw-cache/index.json` および `[hash]_[mtime]_preview.png` を自動特定してインライン表示。
    - **Excalidraw ファイル閲覧**: `.excalidraw.md` ファイルを開いた際、生JSONではなく図面プレビュー画像と「Excalidrawで開く（Port 3001）」リンクを表示（生テキスト表示との切替可能）。
    - **Mermaid ダイアグラム**: Markdown内の ```mermaid ブロックをクライアント側でSVGダイアグラムとして動的レンダリング。ダークテーマにも自動対応。
...
- `docs/table_alignment_fix.md`: テーブルアライメント・CSSスコープ改善仕様
- `docs/obsidian_diagram_rendering.md`: Obsidian図面描画・Excalidraw連携・Mermaid対応仕様
