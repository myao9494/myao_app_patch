...
- **キーボードショートカット**: Ctrl+Left / Ctrl+Down(戻る), Ctrl+Right(進む), Ctrl+Up(上の階層へ), a(フォルダ追加、左/中ペインのみ), r(再読み込み/更新、左/中ペインのみ), n(リネーム、左/中ペインのみ)
...
## 実装済み機能（更新）
 ...
 7. **画像インライン表示と画像ビューワー (Image Viewer)**:
 ...
 8. **左右ペイン同期移動 (Synchronized Browsing)**:
 ...
 9. **検索ボックスの強化**:
    ...
    - 入力中のクリアボタン（`×`）、リアルタイム件数表示バッジ（`X / Y件`）、正規表現トグル（`.*`）を実装。
    - `Escape` キーで検索クエリを全消去し、ファイル一覧テーブルへフォーカスを復帰。
 10. **Markdownビューワ／エディタの検索ボックス (Markdown Search)**:
 ...
 11. **Markdownビューワ／エディタの1行集約ヘッダー＆作業領域最大化**:
 ...
 12. **Markdownプレビュー内の画像表示（相対パス・絶対パス・Obsidian構文対応）**:
 ...
 13. **テキストファイルビューアー／エディタの1行集約ヘッダー＆VS Code風シンタックスハイライト**:
    - `FileEditorModal` のヘッダーを横1行に完全集約（タイトル、スリム検索バー、言語バッジ、行数・保存状態、キャンセル・保存ボタン）。ツールバーやフッターを撤去し、コード編集領域を画面いっぱいに最大化。
    - **VS Code Dark+ 準拠のシンタックスハイライト**: Python（`def`, `class`, 組み込み型/関数, self/cls, デコレータ, 複数行 docstring `"""..."""` 等）をVS Codeと同一の色分けで鮮明に表示。
    - **エディタ内検索**: `Cmd/Ctrl + F`、`Enter`/`Shift + Enter`（循環移動）、リアルタイム件数表示、正規表現トグル、構文ハイライトを壊さない `<mark>` 重ね合わせ表示を実装。
    - `Cmd/Ctrl + S`（保存）、`Tab`/`Shift + Tab`（インデント/アウトデント）等キーボード操作完備。
 14. **メールファイルアイコン (.msg, .eml) のCatppuccinメールアイコン対応**:
 ...
 15. **ファイル一覧テーブルのセル垂直揃え・下線浮き上がり防止 (Table Alignment Fix)**:
