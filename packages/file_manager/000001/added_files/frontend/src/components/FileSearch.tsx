...
import { buildAppPathUrl, buildFullPathUrl, getPdfViewUrl, getHtmlViewUrl, openInObsidian, openInVSCode, openSmart } from "../api/files";
...
import { isHtmlFile } from "../utils/codeFileActions";
...
export function FileSearch({
  ...
  const copyVisibleItemsAsBulletList = useCallback(async () => {
    const markdown = formatItemsAsMarkdownBulletList([...sortedResults.folders, ...sortedResults.files]);
    if (!markdown) {
      showError("コピーできる検索結果がありません");
      setPaneContextMenu(null);
      return;
    ...
    try {
      await navigator.clipboard.writeText(markdown);
      showSuccess("検索結果を箇条書き形式でコピーしました");
    } catch {
      showError("検索結果のコピーに失敗しました");
    }
    setPaneContextMenu(null);
    ...
    >
      ...
      {contextMenu && (
        <ContextMenu
          ...
          onOpenHtmlPreview={isHtmlFile(contextMenu.item.name) ? () => {
            window.open(getHtmlViewUrl(contextMenu.item.path), "_blank", "noopener,noreferrer");
            setContextMenu(null);
          } : undefined}
